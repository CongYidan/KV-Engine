...
public class SsTable implements Closeable {
    ...
    /**
     * filter block（针对data block的布隆过滤器）
     */
    ...
    public Command query(String key) {
        try {
            ...
            //如果布隆过滤器中不存在，那么一定不存在data block中
            if (!existBF) {
                return null;
            }
    ...
    private void initFromIndex(ConcurrentSkipListMap<String, Command> index) {
        try {
            ...
            tableMetaInfo.setFilterBlockLen(tableFile.getFilePointer() - tableMetaInfo.getFilterBlockStart());
