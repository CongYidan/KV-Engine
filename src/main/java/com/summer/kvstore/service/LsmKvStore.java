...
public class LsmKvStore implements KvStore {
     ...
     * compaction执行定时线程池
     */
    private ScheduledExecutorService compactionTimer = Executors.newSingleThreadScheduledExecutor();

    class CompactionTask implements Runnable {
        private LsmKvStore kvStore;

        public CompactionTask(LsmKvStore kvStore) {
            this.kvStore = kvStore;
        }

        @Override
        public void run() {
            LoggerUtil.info(LOGGER, "CompactionTask running,");
            kvStore.getCompactioner().compaction(kvStore.levelMetaInfos, kvStore.nextFileNumber);
        }
    }
    ...
    public LsmKvStore(String dataDir, int storeThreshold, int partSize) {
        try {
            ...
            this.wal = new RandomAccessFile(dataDir + WAL, RW_MODE);
            ...
            for (File file : files) {
                ...
                if (file.isFile() && fileName.endsWith(KVConstants.FILE_SUFFIX_SSTABLE)) {
                    ...
                    if (this.levelMetaInfos.get(level) == null) {
                        tmpLevelSstables = new ArrayList<>();
                        this.levelMetaInfos.put(level, tmpLevelSstables);
                    } else {
                        tmpLevelSstables = this.levelMetaInfos.get(level);
                    }
            ...
            compactionTimer.scheduleAtFixedRate(compactionTask, 1000, 1000, TimeUnit.MILLISECONDS);

            LoggerUtil.info(LOGGER, "LsmKvStore started,levelMetaInfos=" + this.levelMetaInfos.toString());
        } catch (Throwable t) {
            LoggerUtil.error(LOGGER, t, "初始化异常~");
    ...
    public void set(String key, String value) {
        try {
            ...
            //内存表大小超过阈值进行持久化
            if (SerializationUtils.serialize(memtable).length > KVConstants.MAX_SSTABLE_SIZE) {
                switchIndex();
                dumpToL0SsTable();
            }
        } catch (Throwable t) {
    ...
    public Compactioner getCompactioner() {
        return compactioner;
    }
}
